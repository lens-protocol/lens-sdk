...
import { createAccount, fetchAccount, updateAccountFollowRules } from './account';
...
describe('Given a logged-in user', () => {
  describe('When creating two accounts with SimplePaymentRule in native token denomination', () => {
    ...
    beforeAll(async () => {
      const accountResult = await loginAsOnboardingUser().andThen((sessionClient) =>
        createAccount(sessionClient, {
          metadataUri: uri(`data:application/json,${JSON.stringify(firstAccount)}`),
        })
      ...
      const followerRuleResult = await loginAsOnboardingUser().andThen((sessionClient) =>
          ...
          .andThen(sessionClient.waitForTransaction)
          .andThen(() =>
            fetchAccount(sessionClient, { address: accountAddress }).map(nonNullable),
          ),
      );
