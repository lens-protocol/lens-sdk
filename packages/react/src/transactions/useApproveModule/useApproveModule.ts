 ...
 * @category Modules
 * @group Hooks
 * @param args - {@link UseApproveModuleArgs}
 */
export function useApproveModule(
  { limit }: UseApproveModuleArgs = { limit: TokenAllowanceLimit.EXACT },
): UseDeferredTask<
  void,
  | InsufficientGasError
  | PendingSigningRequestError
...
> {
  ...
  const resolveTokenAllowanceRequest = (on: AnyPublication | Profile) => {
    if (on.__typename === 'Profile') {
      return resolveTokenAllowanceRequestForFollow(on, limit);
    }

    invariant(
      session?.authenticated,
      'User must be authenticated to allow to approve the correct contract',
    );
    ...
    return resolveTokenAllowanceRequestForCollect({
      ...
      publicActProxy: config.environment.contracts.publicActProxy,
    });
  };

  return useDeferredTask(async ({ on }) => {
    const request = resolveTokenAllowanceRequest(on);
