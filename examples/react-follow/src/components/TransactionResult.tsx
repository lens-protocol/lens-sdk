...
export function TransactionResult({ loading, txHash, message, error }: TransactionResultProps) {
  if (loading) {
    return (
      <div style={{ padding: '10px', backgroundColor: '#e3f2fd', borderRadius: '4px', marginBottom: '20px' }}>
    ...
    return (
      <div style={{ padding: '10px', backgroundColor: '#e8f5e8', borderRadius: '4px', marginBottom: '20px' }}>
        ...
        <p style={{ margin: '5px 0 0 0', fontSize: '12px', color: '#555' }}>
          Transaction Hash: <code style={{ backgroundColor: '#f5f5f5', padding: '2px 4px', borderRadius: '2px' }}>{txHash}</code>
        </p>
      </div>
    );
  }
